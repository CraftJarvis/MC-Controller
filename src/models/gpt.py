...
from src.models.mha import MHA, ParallelMHA # HJ
from flash_attn.modules.mlp import Mlp, FusedMLP, ParallelFusedMLP
# from flash_attn.modules.block import Block, ParallelBlock
from src.models.block import Block, ParallelBlock # HJ
...
class GPTModel(GPTPreTrainedModel):
    ...
    # HJ
    def forward(self, input_ids, inputs_embeds=None, attention_mask=None, position_ids=None, inference_params=None):
        ...
        if inputs_embeds is None:
            ...
            hidden_states = inputs_embeds # HJ
        if self.parallel_block:
            hidden_states2 = None
        batch, seqlen = hidden_states.shape[:2] # HJ
        residual = None
        # mixer_kwargs = ({'seqlen': input_ids.shape[1]}
        #                 if self.process_group is not None and self.sequence_parallel else {})
        if attention_mask is not None:
            hidden_states, indices, cu_seqlens, max_seqlen_in_batch = unpad_input(hidden_states, attention_mask) # HJ
            mixer_kwargs = ({'seqlen': input_ids.shape[1], 'cu_seqlens': cu_seqlens, 'max_seqlen': max_seqlen_in_batch} # HJ
                if self.process_group is not None and self.sequence_parallel else {'cu_seqlens': cu_seqlens, 'max_seqlen': max_seqlen_in_batch})
        else:
            mixer_kwargs = ({'seqlen': input_ids.shape[1]} 
                if self.process_group is not None and self.sequence_parallel else {}) 
        ...
        if self.prenorm:
            if not self.fused_dropout_add_ln:
                ...
                if not self.parallel_block:
                    # print(f"dropped.shape={dropped.shape}, residual.shape={residual.shape}")
...
class GPTLMHeadModel(GPTPreTrainedModel, GenerationMixin):
    ...
    # def forward(self, input_ids, position_ids=None, inference_params=None):
    def forward(self, input_ids, position_ids=None, inputs_embeds=None, attention_mask=None, inference_params=None, only_return_hidden_states=False):
        """
        ...
        hidden_states = self.transformer(input_ids, position_ids=position_ids, inputs_embeds=inputs_embeds, attention_mask=attention_mask,
                                         inference_params=inference_params)
        if self.project_out is not None:
            hidden_states = self.project_out(hidden_states)
        
        if only_return_hidden_states: 
            return hidden_states
